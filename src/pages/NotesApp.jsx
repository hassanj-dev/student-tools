...
// Colour palettes that work in both light & dark mode using SparkDesk tokens
const NOTE_COLORS = [
        ...
        <div style={{
          ...
          <button onClick={onClose}
            style={{
              ...
              fontSize: "1.2rem", cursor: "pointer", color: "var(--muted)",
              lineHeight: 1,
            }}>✕</button>
        </div>
...
export default function NotesApp() {
  ...
  const addOrUpdate = ({ title, content, colorId }) => {
    if (modal?.note?.id) {
      setNotes((prev) =>
        prev.map((n) => n.id===modal.note.id
          ? { ...n, title, content, colorId, updatedAt: ts() } : n)
      );
    } else {
      setNotes((prev) => [
        { id: Date.now(), title, content, colorId, createdAt: ts(), updatedAt: ts() },
        ...prev,
      ]);
    }
    setModal(null);
...
/>
      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          ...
          {!search && (
            <button className="btn" onClick={() => setModal({ note: null })}>
              + Create First Note
            </button>
          )}
        </div>
      ...
      {notes.length > 0 && (
        <div style={{
          ...
          {NOTE_COLORS.map((c) => {
            const count = notes.filter((n) => n.colorId===c.id).length;
